...
import { Send, RotateCcw, GitBranch, ChevronLeft, ChevronRight, Maximize2, Minimize2 } from "lucide-react";
...
const MAX_DRAG_WIDTH = 1200;

export function ChatPanel({ repoId, wsEnabled = true, onSourceClick }: ChatPanelProps) {
  const [input, setInput] = useState("");
  const [width, setWidth] = useState(DEFAULT_WIDTH);
  const [isExpanded, setIsExpanded] = useState(false);
  const [maximized, setMaximized] = useState(false);
  const { messages, status, rateLimit, sendMessage, clearMessages, reconnect } = useWebSocket(repoId, {
    enabled: wsEnabled,
  });
  ...
  const panelShellClass = cn(
    ...
    maximized
      ? "fixed z-50 top-12 right-0 bottom-0 w-[min(96vw,1400px)] border-l border-zinc-200 dark:border-zinc-800 shadow-2xl"
      : "flex-shrink-0 border-l border-zinc-200 dark:border-zinc-800"
  );
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {(status === "error" || status === "disconnected") && wsEnabled && (
            <button
              type="button"
              onClick={reconnect}
          ...
          {messages.length > 0 && (
            <button
              ...
              className="text-[11px] text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors px-1"
            >
              Clear
            </button>
          )}
          <button
            ...
            className="p-1 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
            title={maximized ? "Exit fullscreen" : "Expand to large panel"}
          >
          ...
          {!maximized && (
            <button
              ...
              className="p-1 rounded text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
              title={isExpanded ? "Collapse width" : "Widen panel"}
            >
      ...
      {status !== "connected" && (
        <div
          className={cn(
            ...
            !wsEnabled && "bg-zinc-50 dark:bg-zinc-900 text-zinc-500",
            wsEnabled && status === "connecting" && "bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400",
            wsEnabled && status === "disconnected" && "bg-zinc-50 dark:bg-zinc-900 text-zinc-400",
            wsEnabled && status === "error" && "bg-red-50 dark:bg-red-900/20 text-red-500"
          )}
        >
          {!wsEnabled && "Chat unlocks when indexing finishes (ready)."}
          {wsEnabled && status === "connecting" && "Connecting..."}
          {wsEnabled && status === "disconnected" && "Disconnected"}
          {wsEnabled && status === "error" && "Connection error — click to retry"}
        </div>
      )}
      ...
      <div className="p-3 border-t border-zinc-200 dark:border-zinc-800">
        ...
        >
          <textarea
            ...
            disabled={!isConnected || isStreaming || !wsEnabled}
            rows={1}
          ...
          <button
            type="button"
            onClick={handleSend}
            disabled={!input.trim() || !isConnected || isStreaming || !wsEnabled}
            className={cn(
              "p-1 rounded transition-colors flex-shrink-0",
              input.trim() && isConnected && !isStreaming && wsEnabled
  ...
  return (
    <>
      {maximized && (
        <button
          type="button"
          className="fixed z-40 top-12 left-0 right-0 bottom-0 bg-black/40 backdrop-blur-[1px]"
          aria-label="Close expanded chat"
          onClick={() => setMaximized(false)}
        />
      )}
      <div className={panelShellClass} style={maximized ? undefined : { width }}>
        {!maximized && (
          <div
        ...
        {maximized && (
          <div
            className="w-1 flex-shrink-0 cursor-col-resize hover:bg-[#1D9E75]/40 transition-colors"
            title="Drag disabled in fullscreen — use the minimize control"
          />
        )}
        {panelBody}
      </div>
